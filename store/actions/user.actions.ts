...
export const signup = (email: string, password: string) => {
  return async (dispatch: any, getState: any) => {
    ...
    const response = await fetch(
      'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBs9GdYK7ZmIGi5EYOoKk3eKyTOU5XU2A4',
      {
        method: 'POST',
        headers: {
...
export const login = (email: string, password: string) => {
  return async (dispatch: any, getState: any) => {
    const response = await fetch(
      'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBs9GdYK7ZmIGi5EYOoKk3eKyTOU5XU2A4',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
